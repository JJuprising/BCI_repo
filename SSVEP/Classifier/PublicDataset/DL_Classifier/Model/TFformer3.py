...
class SSVEPformer(nn.Module):
    def __init__(self, depth, fre_channel,attention_kernal_length, chs_num, class_num, dropout):
        super().__init__()
        token_num = chs_num * 2
        token_dim = 560
        self.to_patch_embedding = nn.Sequential(
            nn.Conv1d(chs_num, fre_channel, 1, padding=1 // 2, groups=1),
            nn.LayerNorm(token_dim),
            nn.GELU(),
            nn.Dropout(dropout)
        )

        self.transformer = convTransformer(depth, fre_channel, token_dim, attention_kernal_length, dropout)
...
class TFformer3(Module):
    ...
    def __init__(self, T, depth, heads, chs_num, class_num, tt_dropout, ff_dropout, dim_thead=8, dim_fhead=8, dim=220):
        ...
        # SSVEPformer
        self.subnetwork = SSVEPformer(depth=depth, fre_channel=fre_channel,attention_kernal_length=31, chs_num=chs_num, class_num=class_num,
                                      dropout=ff_dropout)
        ...
        for _ in range(1):
            self.crossAttentionEncoder.append(ModuleList([
                CrossAttention(fre_channel, time_channel, 8, 8, 8),
                nn.LayerNorm(fre_channel),
                FeedForward(fre_channel, hidden_dim=16, dropout=ff_dropout),
                nn.LayerNorm(fre_channel)
            ]).to(device))
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.Linear(560 * fre_channel, class_num * 6),
    ...
    def forward(self, x):
        ...
        # x_t = butter_bandpass_filter(x_t, 8, 64, self.fs)
        # x_t = torch.from_numpy(x_t).to(x.device).to(torch.float32)
