...
class AITST(nn.Module):
    def __init__(self, input_size, patch_size, channels, num_classes, model_dim,
                 emb_dropout=0.1, pool='cls', dim_head=64, channel_rate=5,
                 depth=8, heads=8, mlp_dim=512, dropout=0.1, model_structure="sa+ca"):
...
if __name__ == '__main__':
    ...
    model = AITST(input_size=(125, 6), patch_size=(5, 2), num_classes=123, model_dim=512, depth=6, heads=8,
                  mlp_dim=1024,
                  dropout=0.1,
                  emb_dropout=0.1, channels=32).to('cuda')
    output = model(input)
    print(output.shape)
